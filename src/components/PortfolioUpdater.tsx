  ...
  return (
    <Card className="bg-white border-slate-200 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-lg font-semibold text-slate-900">Update Portfolio</h4>
        {!showAddForm && (
          <Button
            size="sm"
      ...
      {message && (
        <div className={`mb-4 p-3 rounded-lg ${message.type === 'success' ? 'bg-green-50 border border-green-200 text-green-700' : 'bg-red-50 border border-red-200 text-red-700'} text-sm`}>
          {message.text}
        </div>
      )}
      ...
      {showAddForm && (
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg space-y-3">
          <h5 className="font-semibold text-slate-900">Add New Stock</h5>
          
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-2">Search Stock</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"
              />
            </div>
            
            {isSearching && (
              <div className="mt-2 text-sm text-slate-600">Searching...</div>
            )}
              ...
              <div className="mt-2 space-y-2 max-h-40 overflow-y-auto">
                {searchResults.map((result) => (
                  <button
                    ...
                    className="w-full text-left p-2 bg-white border border-slate-200 rounded hover:bg-slate-50 transition text-sm"
                  >
                    <p className="font-medium text-slate-900">{result.symbol}</p>
                    <p className="text-slate-600 text-xs">{result.name}</p>
                  </button>
                ))}
              </div>
            )}
          ...
          {selectedStock && (
            <>
              <div className="p-2 bg-white border border-slate-300 rounded">
                <p className="font-medium text-slate-900">{selectedStock.symbol}</p>
      ...
      <div className="space-y-3">
        {holdings.length === 0 ? (
          ...
          holdings.map((holding) => (
            <div key={holding.id} className="p-4 bg-slate-50 rounded-lg border border-slate-200">
              {editingId === holding.id ? (
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-slate-900 font-medium">{holding.name || holding.symbol}</p>

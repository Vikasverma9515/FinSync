...
export default function Pricing() {
    return (
        <section id="pricing" className="py-24 bg-navy-950 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                    {plans.map((plan, index) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className={`relative bg-navy-800/50 backdrop-blur-sm border ${plan.popular ? 'border-teal-400 shadow-[0_0_30px_rgba(100,255,218,0.1)]' : 'border-slate-700/50'} rounded-2xl p-8 flex flex-col`}
                        >
                            {plan.popular && (
                                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-teal-400 text-navy-900 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide">
                                    Most Popular
                                </div>
                            )}
